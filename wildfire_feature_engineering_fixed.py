...
def print_header():
    ...
    print("="*60)
    print("🔥 WILDFIRE FEATURE ENGINEERING PIPELINE - SERVER VERSION")
    print("="*60)
...
def load_and_explore_dataset(data_path):
    ...
    try:
        ...
        if target_col in df.columns:
            ...
            print(f"Target variable distribution (before processing):")
            print(target_before)
            ...
            print(f"\nBinary target 'fire_spread' distribution:")
            print(target_after)
            print(f"Class balance: {target_after[1]/len(df)*100:.1f}% positive class")
        ...
        print("Dataset summary saved to wildfire_results/dataset_summary.txt")
...
def handle_missing_values(df, missing_threshold=20):
    ...
    if len(remaining_missing) > 0:
        ...
        if numeric_features:
            ...
            df_cleaned[numeric_features] = df_cleaned[numeric_features].fillna(method='ffill').fillna(0.0)
...
def apply_transformations_and_scaling(df):
    ...
    for feature in numeric_features:
        ...
        outliers_before = ((df_transformed[feature] < lower_bound) | 
                          (df_transformed[feature] > upper_bound)).sum()
...
def main():
    ...
    data_path = "features_array.csv"  # Update this path on the server
    ...
    try:
        ...
        # Step 4: Apply transformations (scaling removed for optimization)
        df_final = apply_transformations_and_scaling(df_filtered)
